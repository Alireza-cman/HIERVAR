...
def fit(X, num_features = 10_000, max_dilations_per_kernel = 32):
    ...
    random_size = np.random.randint(1,5,len(biases)).astype(np.int32)
    
    return num_channels_per_combination, channel_indices, dilations, num_features_per_dilation,random_size, biases
...
@njit("float32[:,:](float32[:,:,:],Tuple((int32[:],int32[:],int32[:],int32[:],int32[:],float32[:])))", fastmath = True, parallel = True, cache = True)
def transform(X, parameters):

    num_examples, num_channels, input_length = X.shape

    num_channels_per_combination, channel_indices, dilations, num_features_per_dilation,random_size, biases = parameters
...
# def RASTERM(x_train,y_train, x_test, y_test,n_features = 10_000):
#     parameter = fit(x_train,num_features =n_features)
#     x_train_trans_org = transform(x_train, parameter)
#     x_test_trans_org = transform(x_test, parameter)
